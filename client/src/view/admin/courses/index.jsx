...
const Filter = ({setParams}) => {
    ...
    return (
        <DataFilter filter={filter} setFilter={setFilter} setParams={setParams} className={'xs:w-full lg:w-9/12'}>
            ...
            />
            <InputFormV2 type="number" value={filter.fromPrice}
                         onChange={(e) => setFilter({...filter, fromPrice: e.target.value})} label="Giá từ"/>
            <InputFormV2 type="number" value={filter.toPrice}
                         onChange={(e) => setFilter({...filter, toPrice: e.target.value})} label="Giá đến"/>
            <SelectFormV2
                value={filter.type}
