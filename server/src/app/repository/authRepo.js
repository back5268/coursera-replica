...
export const createUserRp = async ({ fullName, username, email, password, bio, address, status, role }) => {
  ...
  const data = await addUserMd({ fullName, username, email, password: newPassword, bio, address, status, role });
  return { data };
};
