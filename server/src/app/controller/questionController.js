import {
    addQuestionValid,
    updateQuestionValid,
    detailQuestionValid, listQuestionValid, exportQuestionValid
} from '@lib/validation';
import {
    addQuestionMd,
    deleteQuestionMd,
...
export const importQuestion = async (req, res) => {
    try {
        if (req.file) {
            ...
            if (data && data.length > 0) {
                for (let datum of data) {
                    newData.push([datum.stt, datum.lessonCode, datum.content, datum.answers, datum.answer, datum.mess ? 'Thất bại' : 'Thành công', datum.mess || 'Thêm câu hỏi thành công'])
                }
            }
            res.status(200)
...
export const exportQuestion = async (req, res) => {
    try {
        ...
        if (data && !mess) {
            ...
            const newData = []
            newData.push(['STT', 'Bài giảng', 'Mã bài giảng', 'Câu hỏi', 'Câu trả lời', 'Đáp án', 'Thời gian tạo', 'Thời gian cập nhật', 'Trạng thái'])
            if (documents && documents.length > 0) {
                ...
                for (let datum of documents) {
                    ...
                    newData.push([index, lesson.title, lesson.code, datum.content, answers, answer,
